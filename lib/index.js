...
function xrequire(name){
    if(develPath){
        var devPath = pathModule.resolve(develPath, name) ;
...
function getFullPath(path, calledFrom) {
    ...
    var localModuleName = pathModule.join(pathModule.dirname(calledFrom), path);
    try {
...
function newLoader(request) {
    ...
    if(fullFilePath.startsWith(currentRoot) || pathModule.resolve(fullFilePath).startsWith(currentRoot)){
...
exports.chroot = function(newRoot){
    currentRoot = pathModule.resolve(newRoot);
